...
fun averageStockPrice(stockPrices: List<Pair<String, Double>>): Map<String, Double> {
    val summap = mutableMapOf<String, MutableList<Double>>()
    val map = mutableMapOf<String, Double>()
    for ((a, b) in stockPrices) {
        if (a !in summap.keys) {
            summap.put(a, mutableListOf(b, 1.0))
        } else {
            summap.getValue(a)[0] += b
            summap.getValue(a)[1] += 1.0
        }
    }
    for (i in summap) {
        map.put(i.key, i.value[0] / i.value[1])
    }
    return map
}
...
fun findCheapestStuff(stuff: Map<String, Pair<String, Double>>, kind: String): String? {
    var p: String? = null
    val rez = mutableMapOf<String, Pair<String, Double>>()
    for ((item, price) in stuff) {
        if (price.first == kind) {
            if (price.first !in rez) {
                rez.put(price.first, Pair(item, price.second))
                p = item
            } else if (price.second < rez.getValue(price.first).second) {
                rez.put(price.first, Pair(item, price.second))
                p = item
            }
            else p == null
...
fun findSumOfTwo(list: List<Int>, number: Int): Pair<Int, Int> {
    ...
    for (i in list) {
        for (j in list) {
            if (i + j == number && i != j) {
                if (i < j) n = Pair(list.indexOf(i), list.indexOf(j))
                else n = Pair(list.indexOf(j), list.indexOf(i))
            }
        }
    }
    return n
